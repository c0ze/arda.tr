...
let renderCard = item =>
  <Card key={item.title} className="overflow-hidden border-border bg-card hover:border-primary/30 hover:shadow-[0_0_24px_-6px_hsl(var(--primary)/0.2)] transition-all group">
    ...
    <CardHeader className="pb-2">
      ...
      <CardTitle className="flex items-center justify-between text-base">
        ...
        >
          ...
          <span className="sr-only"> {`Visit ${item.title}`->React.string} </span>
        </a>
      </CardTitle>
    </CardHeader>
    <CardContent>
...
@react.component
let make = () => {
  let projects: array<portfolioItem> = [
    {
  ...
  <section id={SiteConfig.portfolio} className="py-24 px-6 bg-background">
    <div className="max-w-5xl mx-auto space-y-16">
      <div className="space-y-8">
        <div className="text-center space-y-3">
          <span className="font-mono text-xs text-primary/70 tracking-[0.2em] uppercase">
            {"02 — Projects"->React.string}
          </span>
          <h2 className="text-2xl md:text-3xl font-bold text-foreground">
            {"Projects"->React.string}
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto">
            {"A collection of digital products and interactive experiences I've built."->React.string}
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects->Array.map(renderCard)->React.array}
        </div>
      </div>
      ...
      <div className="space-y-8">
        <div className="text-center space-y-3">
          <span className="font-mono text-xs text-primary/70 tracking-[0.2em] uppercase">
            {"03 — Games"->React.string}
          </span>
          <h2 className="text-2xl md:text-3xl font-bold text-foreground">
            {"Games"->React.string}
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto">
            {"A collection of web-based games I've developed."->React.string}
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {games->Array.map(renderCard)->React.array}
        </div>
      </div>
      ...
      <div className="space-y-8">
        <div className="text-center space-y-3">
          <span className="font-mono text-xs text-primary/70 tracking-[0.2em] uppercase">
            {"04 — Tools"->React.string}
          </span>
          <h2 className="text-2xl md:text-3xl font-bold text-foreground">
            {"Tools"->React.string}
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto">
            {"Utilities and tools I've developed to improve workflows."->React.string}
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tools->Array.map(renderCard)->React.array}
        </div>
      </div>
    </div>
  </section>
